...
PXSgl = function(df,
  ...
  logger::log_info(paste('intiating group lasso PXS procedure with', length(X), 'variables'))
  ...
  logger::log_info(paste('cross validation with',folds,'folds'))
  ...
  logger::log_info(paste('the  min lamda  is:', cv_fit$lambda[lamb]))
  ...
  if(length(catnames) ==0 & length(contnames)==0){
    logger::log_error('group lasso did not select any variables, procedure terminated')
    stop()
  }
  ...
  logger::log_info('recalibrating model in group B...')
  ...
  logger::log_info(paste(nrow(keepBC),'individuals remain'))
